...
Page({
    ...
    onLoad: function (options) {
        ...
        var that = this,
            ...
            // 2019-11-21 ：增加判断用户是否绑定手机号码
            userInfo = wx.getStorageSync("userInfo"),
            tel = userInfo.CellPhone,
            installmentPayId = options.installmentPayId;
            // 2019-12-18 : 新增分期Id
            ...
            success: function (result) {
              ...
              console.log(result.data)
                if (result.success) {
                    const r = result.data;
                    if(r.errorCode == "0"){
                        app.showErrorModal(r.errorMsg, function (res) {
                            if (res.confirm) {
                                wx.navigateTo({
                                    url: '../productdetail/productdetail?id=' + r.productId
                                })
                            }
                        });
                    }else{
                    ...
                    that.setData({
                        ...
                        pageLoaded: true,
                        installmentPayDetailInfo: r.installmentPayDetailInfo,
                        installmentPayId: installmentPayId,
                        EndDate:r.endDate
                  });
    ...
    reGetOrderInfo: function () {
        ...
        wx.request({
            ...
            data: {
                ...
                InstallmentPayId: tmp.installmentPayId
            },
            success: function (result) {
                result = result.data;
                if (result.success) {
    ...
    caleOrderAmount: function () {
        ...
        if (that.data.installmentPayId >0){
          total = that.data.orderAmount;
        }
        total = total + invoice;
    ...
    submitOrder: function (e) {
        ...
        app.getOpenId(function (openid) {
            ...
            var parameters = {
                ...
                InstallmentPayId: that.data.installmentPayId,
                IsLoan:1,
                EndDate: that.data.EndDate,
                IsLease: that.data.IsLease == 'true' ? JSON.parse(that.data.IsLease) : false
            };
            var orderSubmitUrl = 'Order/SubmitOrder';
            if (that.data.isGroup) {
                orderSubmitUrl = 'Order/PostSubmitFightGroupOrder';
            }
            var IsLease = that.data.IsLease
                ...
                if (sd.success) {
                    if (that.data.isGroup) {
                        ...
                        if (sd.data.RealTotalIsZero) {
                            ...
                            app.orderPay(orderId, 0, false, IsLease);
                        }
                    } else {
