...
Page({
  data: {
    ...
    // 2020.06.09 续期
    RenewalOrder: function (e){
      var that = this;
      var productId = that.data.OrderInfo.LineItems[0].ProductId;
      wx.navigateTo({
            ...
            wx.request({
                ...
                success: function (result) {
                    ...
                    if (result.success) {
                      ...
                      console.log(r)
                      var newList = "";
                      var bureauTime = util.compareTime(new Date().getTime(), new Date(r.OrderEndDate).getTime())
                      console.log(bureauTime)
                      if (bureauTime.Days <= 365){
                        that.setData({
                          isRenewal: true
                        })
                      }
                      ...
                      var newOrderInstallmentPay = [];
                      for (var index in r.OrderInstallmentPay){
                        var payType = r.OrderInstallmentPay[index].PayState
                        if (payType == false){
                          newOrderInstallmentPay.push(r.OrderInstallmentPay[index])
                        }
                      }
                      that.setData({
                        OrderInfo: r,
                        ...
                        pageLoaded: true,
                        OrderInstallmentPay: r.OrderInstallmentPay,
                        thisOrderInstallmentPay: newOrderInstallmentPay[0]
                      })
                    }
                    else if (result.code == '502') {
                        wx.navigateTo({
