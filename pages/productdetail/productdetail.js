...
Page({
    data: {
        ...
        agingIndex:0,
        agingPrice:0,
        IsLease: false,     // 是否为租赁商品
        isReadingLease: false,     //是否阅读租赁协议
        isShowLease: false      // 租赁协议弹窗
    },
  ...
  clickSku: function (e) {
    ...
    doCommit: function(e) {
        var option = e.currentTarget.dataset.option,
            that = this;
        if (that.data.IsInstallmentPay) {
          if (that.data.agingId == 0){
            return app.showErrorModal("请选择支付方式");
          }
        }
        if(that.data.IsLease){
            if(!that.data.isReadingLease){
                app.showErrorModal("未同意租赁协议");
                return;
            }
        }
        ...
        if (option == 'buy') {
            app.getOpenId(function(openid) {
                wx.redirectTo({
                  url: '../submitorder/submitorder?productsku=' + skuid + '&buyamount=' + amount + '&frompage=1' + '&isStore=' + that.data.isStore + '&producttype=' + that.data.producttype + '&shopBranchId=' + that.data.shopBranchId + '&installmentPayId=' + that.data.agingId + '&IsLease=' + that.data.IsLease
                })
            });
